...
(function () {
  ...
  function renderMethod() {
    ...
    var rows = [
      ...
      ["Classifier model", m.llm_model + ", " + m.llm_calls_total + " calls to date, daily ceiling " + m.llm_daily_ceiling + (m.llm_ceiling_days && m.llm_ceiling_days.length ? ", draw capped on " + m.llm_ceiling_days.map(function (d) { return d + " (" + ((m.llm_calls_by_day || {})[d] || 0) + " calls made)"; }).join(", ") : "") + ((m.llm_sampling_days || []).length ? "; stratified draws on " + m.llm_sampling_days.length + " days" : "") + (m.llm_budget ? "; estimated spend " + m.llm_budget.month + ": $" + m.llm_budget.estimated_usd + " of a $" + m.llm_budget.budget_usd + " monthly budget" + (m.llm_budget.daily_cap_today !== null && m.llm_budget.daily_cap_today !== undefined ? ", which allows " + m.llm_budget.daily_cap_today + " batched calls today" : "") : "")],
